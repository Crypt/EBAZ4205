...
ENTITY quadrature_decoder IS
	GENERIC(
		positions					:	INTEGER := 10000; 		--size of the position counter (i.e. number of positions counted)
	...
	PORT(
		...
		phase           :   IN          STD_LOGIC;                                      --change direction
		set_origin_n	:	IN			STD_LOGIC;  									--active-low synchronous clear of position counter
		direction		:	BUFFER		STD_LOGIC;										--direction of last change, 1 = positive, 0 = negative
		update          :   OUT         STD_LOGIC;
		value		    :	OUT   	    INTEGER RANGE 0 TO positions-1 := 0);	        --current position relative to index or initial value
...
BEGIN
	...
	BEGIN
		IF(clk'EVENT AND clk = '1') THEN												--rising edge of system clock
			...
			-- swap a & b, and invert polarities 
			a_new <= a_new(0) & not b;													--shift in new values of 'a'	
			b_new <= b_new(0) & not a;													--shift in new values of 'b'
			...
			IF(set_origin_n_int = '0') THEN												--inital position is being set
				position <= 0;	
				value <= 0;														--clear position counter
       			update <= '1';
			ELSIF(debounce_cnt = debounce_time											--debounce time for a and b is met
				...
				direction <= phase XOR (b_prev XOR a_new(1));									--update the direction
--			    IF( a_new(1)='0' AND b_new(1)='0' ) THEN							    --detent position
        			update <= '1';
                    IF(direction = '1') THEN								            --clockwise direction
